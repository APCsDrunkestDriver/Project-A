...
class Ball:
    ...
    def serve(self):
        self.x = window_width // 2
        self.y = window_height // 2
        self.xVel = random.choice([-10.0, 10.0])
        self.yVel = random.uniform(-10.0, 10.0)
    ...
    # Top or bottom wall collision
        ...
        for paddle in paddles:
            if paddle.x <= self.x <= paddle.x + paddle.width and \
                ...
                # changes the vilocity and direction of the ball depending on where you hit it.
                # hitting further towards the edges results in a higher vilocity and more angle.
                self.xVel *= -1


                
                ball_position_on_paddle = paddle.y+45-self.y
                self.yVel = ball_position_on_paddle/-4 # creates y velocity

                self.xVel = 10-self.yVel # creates x velocity using y velocity
    ...
    # Top or bottom wall collision
        ...
        for paddle in paddles:
            if paddle.x <= self.x <= paddle.x + paddle.width and \
                ...
                ball_position_on_paddle = paddle.y+45-self.y
                self.yVel = ball_position_on_paddle/-2 # creates y velocity
